...
from src.EQLdiv.utils import number_of_positional_arguments
...
class reg_div(object):
    ...
    def __call__(self, numerator, denominator):
        ...
        div = tf.math.reciprocal(tf.abs(denominator) + 1e-10)
        output = numerator * div * mask
        P_theta = tf.maximum((div_thresh - denominator), 0.0)  # equation 5 in paper
        tf.compat.v1.add_to_collection('Threshold_penalties', P_theta)
        return output


# ---- numerically safe TF ops ----
def tf_log_safe(x, eps=1e-12):
    # log(|x| + eps) avoids NaNs for x<=0
    x = tf.cast(x, tf.float32) if x.dtype.is_integer else x
    return tf.math.log(tf.abs(x) + tf.cast(eps, x.dtype))

def tf_exp_safe(x, clip=50.0):
    # clip to avoid overflow -> inf
    x = tf.cast(x, tf.float32) if x.dtype.is_integer else x
    x = tf.clip_by_value(x, tf.cast(-clip, x.dtype), tf.cast(clip, x.dtype))
    return tf.exp(x)

# ---- SymPy counterparts ----
def sp_log_safe(x, eps=sp.Float("1e-12")):
    return sp.log(sp.Abs(x) + eps)

# Optional: exp is already safe symbolically; overflow only happens numerically.
def sp_exp_safe(x):
    return sp.exp(x)
...
dict_of_ops = {
    "multiply": (tf.multiply, sp.Symbol.__mul__),
    "sin":      (tf.sin, sp.sin),
    "cos":      (tf.cos, sp.cos),

    "id":       (tf.identity, lambda x: x),

    "sub":      (tf.subtract, sp.Symbol.__sub__),

    "log":      (tf_log_safe, sp_log_safe),
    "exp":      (tf_exp_safe, sp_exp_safe),
...
class EQL_Layer(object):
    ...
    def get_matmul_output(self, data, l1_reg_sched, l0_threshold):
        ...
        if (not hasattr(self, "_dense")) or (self._dense is None):
            k_init = tf.keras.initializers.RandomUniform(
                minval=-self.w_init_scale, maxval=self.w_init_scale, seed=self.seed
            )
            self._dense = tf.keras.layers.Dense(
                self.matmul_output_dim,
        ...
        new_kernel = kill_small_elements(self._dense.kernel, l0_threshold)
        new_bias = kill_small_elements(self._dense.bias, l0_threshold)

        self._dense.kernel.assign(new_kernel)
        self._dense.bias.assign(new_bias)

        return self._dense(data)
