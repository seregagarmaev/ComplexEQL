...
from src.EQLdiv.timeout import time_limit, TimeoutException
from src.EQLdiv.utils import generate_arguments, yield_with_repeats, weight_name_for_i


class EvaluationHook(tf.compat.v1.train.SessionRunHook):
    ...
    def before_run(self, run_context):
        if self.iteration == 0:
            ...
            tens = {}
        return tf.compat.v1.train.SessionRunArgs(fetches=tens)
...
def complexity_of_layer(fns, in_biases, in_weights, out_weights, thresh):
    ...
    """
    in_weight_sum = np.sum(np.abs(in_weights), axis=0) + in_biases
...
def build_symbolic_outputs(kernels, biases, fns_list, round_decimals=3, simplify=True):
    """
    Build the symbolic outputs (one SymPy expression per output dimension) from the trained network.

    Parameters
    ----------
    kernels : list[np.ndarray]
        List of weight matrices (input_dim x output_dim per layer).
    biases : list[np.ndarray]
        List of bias vectors (output_dim per layer).
    fns_list : list[list[tuple]]
        Layer function structure as returned by EQL_Layer.get_fns().
    round_decimals : int
        Rounding applied to SymPy Float constants.
    simplify : bool
        If True, attempt (time-limited) simplification.

    Returns
    -------
    list[sympy.Expr]
        Symbolic expressions for each output node (e.g., [y0, y1, ...]).
    """
    in_nodes = get_symbol_list(kernels[0].shape[0])
    res = in_nodes
    ...
    out_exprs = []
    for result in res:
        rounded = round_sympy_expr(result, round_decimals)
        if simplify:
            try:
                rounded = proper_simplify(rounded)
            except (TimeoutException, RecursionError):
                pass
        out_exprs.append(rounded)
