...
def getEdges(matrix, inputnames, outputnames, thresh=0.1):
    ...
    while not it.finished:
        if np.abs(it[0]) > thresh:
            edges.append((
                inputnames[it.multi_index[0]],
                outputnames[it.multi_index[1]],
                np.round(it[0].item(), 2),
            ))
        it.iternext()
...
def functionGraph1H(classifier, thresh=0.1):
    return functionGraph(classifier, thresh)
...
def functionGraph(classifier, thresh=0.1):
    ...
    for layer_idx in range(len(classifier.hidden_layers) + 1):
        if layer_idx == 0:
            inp = names_in
        else:
            inp = classifier.hidden_layers[layer_idx - 1].getNodeFunctions()

        if layer_idx == len(classifier.hidden_layers):
            if shortcuts:
                inp = []
                for hidden_layer in classifier.hidden_layers:
                    inp.extend(hidden_layer.getNodeFunctions())
            ...
            out = classifier.hidden_layers[layer_idx].getWeightCorrespondence()

            ps = classifier.hidden_layers[layer_idx].get_params()
            W = ps[0]
            b = ps[1]
    ...
    for n in arity2:
        dot.node(
            ...
            arity2node(
                ...
                str(np.round(bias_dict.get(n + ':2', 0), 2)),
            ),
            shape='plaintext',
        )
